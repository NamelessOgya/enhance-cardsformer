...
def act(i, device, free_queue, full_queue, model, buffers, flags, stop_event):
    """
    ...
    # checkpoint_states = torch.load(prediction_model_name, map_location='cpu')['model_state_dict'] 
    ...
    # 进行一局游戏
        ...
        if stop_event.poll():  # メッセージが届いているか確認
            message = stop_event.recv()  # メッセージを受け取る
            print("msg_found")
            if message == "STOP":
                print("Child process stopping...")
                break

    print("act func is done")
