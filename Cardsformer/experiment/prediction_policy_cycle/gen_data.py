...
def gen_data(prediction_data_load_path, train_data_limit):
    ...
    for j in range(10):
        ...
        while True:
            while True:
                ...
                datalen += len(data['hand_card_names'])
                if done or datalen >= train_data_limit:
                    break
            if datalen >= train_data_limit:
                break
            
        np.save(prediction_data_load_path + '/off_line_data' + str(j) + '.npy', data)
