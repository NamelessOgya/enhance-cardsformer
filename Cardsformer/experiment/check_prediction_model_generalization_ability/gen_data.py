# python -m experiment.check_prediction_model_generalization_ability.gen_data
...
log.info('train Data generation begins.')
for j in range(10):
    game = Hearthstone(random_cards=True, deck_mode="train")
    ...
    for i in range(1000):
        while True:
            ...
            if done:
                break
    np.save('./experiment/check_prediction_model_generalization_ability/off_line_data/off_line_data' + str(j) + '.npy', data)
    log.info('Round {} completed'.format(j))


log.info('test Data generation begins.')
for j in range(1):
    game = Hearthstone(random_cards=True,deck_mode="test")
    env = Environment(game)
    data = {
        'hand_card_names': [],
        'minion_names': [],
    ...
    for i in range(1000):
        while True:
            ...
            if done:
                break
    np.save('./experiment/check_prediction_model_generalization_ability/off_line_data/test_data' + str(j) + '.npy', data)
    log.info('Round {} completed'.format(j))
